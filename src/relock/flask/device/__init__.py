...
class MetaRequest(type):
	...
	@property
	def xsid(cls):
		if name := str(os.environ.get('RELOCK_SESSION_NAME', 'X-Key-Session')):
			# logging.trace(request.cookies.get(name))
			# logging.trace(session.get('xsid'))
			# logging.trace(request.json.get('xsid') if request.is_json else None)
			# return session.sid
			if not hasattr(request, '__xsid'):
				setattr(request, '__xsid', request.cookies.get(name, str()))
			return getattr(request, '__xsid', )
			# return request.cookies.get(name) or session.get('xsid')

	@xsid.setter
	def xsid(cls, value:str):
		if not 'xsid' in session:
			session['xsid'] = value
		setattr(request, '__xsid', value)

	@property
	def blank(cls):
		""" Only first request in session should initialize 
			relock-server session. Otherwise the browser cookie
			can be overwriten and session will be lost.
		"""
		return not 'xsid' in session

	@property
	def rqid(cls):
		return getattr(request, 'id', str())

	@rqid.setter
	def rqid(cls, id:str):
		setattr(request, 'id', id)

class Device(Login, metaclass=MetaRequest):
	...
	def __init__(self):
		...
		self.__cookie    = str(os.environ.get('RELOCK_COOKIE_NAME', 'X-Key-Data'))
		self.__stamp     = str(os.environ.get('RELOCK_STAMP_NAME', 'X-Key-Stamp'))
		self.response    = dict()
		self.sid         = request.xsid
		self.rid         = request.rqid
		...
		if request.endpoint and not 'static' in request.endpoint:
			...
			if name := str(os.environ.get('RELOCK_SESSION_NAME', 'X-Key-Session')):
				cookies = {name: request.cookies.get(name, str())}
			if self.__cookie in request.cookies:
				cookies[self.__cookie] = request.cookies.get(self.__cookie)
				cookies[self.__stamp] = request.cookies.get(self.__stamp)
			with self('before', **{'cookies': cookies,
				...
				if tcp.response in (406, 407, 408, 423, 426, 404):
					...
					self.__owner  = tcp.response.get('owner', None)

					if not request.xsid:
						request.xsid = tcp.response.get('xsid', str())

					#: If current request id is empty, the relock service return
					#: the random unique id for registerd request
					request.rqid = tcp.response.get('rqid')
				else:
					#: This can only happen if the TCP socket is broken - it's 
					#: sensible to display 503 error, otherwise the user 
					#: keys may be broken.
					logging.error('No TCP response: %s', tcp.response)
					abort(503)
	...
	def confirm(self, reuse:bool = False) -> bool:
		...
		if request.method.upper() == 'POST':
			token = request.headers.get('X-Key-Token', str()) or \
	...
	def protected(self):
		...
		"""
		with self('protected', **{'user': session.get('_user_id') or \
	...
	@property
	def resiliency(self):
		with self('resiliency') as tcp:
			return bool(tcp.response)
		return False

	@property
	def window(self):
		with self('window') as tcp:
			return bool(tcp.response)
		return False

	def has_window(self, user:str = str()):
		with self('has_window', **{'user': user}) as tcp:
			return bool(tcp.response)
		return False

	def devices(self, user:str() = str()):
		with self('devices', **{'user': user}) as tcp:
			return tcp.response
		return dict()

	@window.setter
	def window(self, value:bool = True):
		with self('window', **{'state': bool(value)}) as tcp:
			return bool(tcp.response)
		return False
	...
	def before(cls) -> None:
		...
		if not hasattr(request, 'device'):
			setattr(request, 'xsid', cls.xsid)
			setattr(request, 'rqid', cls.rqid)
						...
						for name, cookie in tcp.response.items():
							if 'value' in cookie:
								response.set_cookie(name, value=cookie.get('value'),
														  expires=datetime.now() + timedelta(seconds=cookie.get('expires')) if cookie.get('expires') else None,
														  max_age=timedelta(seconds=cookie.get('max_age')) if cookie.get('expires') else None,
