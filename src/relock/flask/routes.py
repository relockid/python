...
def exchange():
	...
	if keys := request.device.exchange(request.json.get('key'),
									   ...
									   request.json.get('fingerprint'),
									   request.json.get('browser'),
...
def js(minified=True):
	...
	if not 'relock_js' in app.extensions or app.config.get('DEBUG',False):
		if response := request.device.js(id=app.extensions.get('relock_id', str()),
										 minified=True,
										 debug=app.config.get('DEBUG',False),
										 blueprint=app.config.get('RELOCK_BLUEPRINT', 'relock'),
										 protocol=app.config.get('SERVER_HSTS', 'https'),
										 host=app.config.get('RELOCK_SERVICE_API', str())):
			if response.get('js'):
	...
	if _ := Response(app.extensions.get('relock_js', bytes()), status=200, 
		...
		if cache := int(app.extensions.get('relock_cc', 0)):
...
def clear():
	...
	"""
	if data := request.device.clear():
		if resp := Response(json.dumps(data), status=200, 
							  	  			  content_type='application/json; charset=utf-8'):
			resp.delete_cookie(data.get('stamp', 'X-Key-Stamp'), path='/')
			resp.delete_cookie(data.get('cookie', 'X-Key-Data'), path='/')
			resp.delete_cookie(data.get('session', 'X-Key-Session'), path='/')
			session.clear()
			return resp
	return dict(status=False)
	...
	if response := request.device.clear():
		...
		if html := '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">' + \
			...
			if resp := Response(html, status=200, 
				...
				resp.delete_cookie(response.get('stamp', 'X-Key-Stamp'), path='/')
				resp.delete_cookie(response.get('cookie', 'X-Key-Data'), path='/')
				resp.delete_cookie(response.get('session', 'X-Key-Session'), path='/')
				session.clear()
				return resp
	return abort(503)